...
BOOST_AUTO_TEST_CASE_TEMPLATE(GivenMap_CheckingRotation,
                              K,
                              TestedKeyTypes)
{
  const Map<K> map = { { 42, "Alice" }, { 52, "Bob" }, { 62, "Romuald"} };

  auto it = map.begin();

  BOOST_CHECK(it->first == 42);
}

BOOST_AUTO_TEST_CASE_TEMPLATE(GivenMap_RemoveByIterator,
                              K,  
                              TestedKeyTypes)
{
  Map<K> map = { { 27, "Bob" }, { 63, "Alice"} };

  auto it = map.begin();

  map.remove(it);

  thenMapContainsItems(map, { { 63, "Alice" } });
}

BOOST_AUTO_TEST_CASE_TEMPLATE(GivenMap_CheckIterating,
                              K,  
                              TestedKeyTypes)
{
  Map<K> map = { { 27, "Bob" }, { 63, "Alice" }, { 84, "Romuald" } };
...
BOOST_AUTO_TEST_CASE_TEMPLATE(GivenMap_CheckingIfTheSame,
                              K,  
                              TestedKeyTypes)
{
  Map<K> map = { { 27, "Bob" }, { 63, "Alice" }, { 84, "Romuald" } };
  Map<K> other = { { 84, "Romuald" }, { 63, "Alice" }, { 27, "Bob" } };

  thenMapContainsItems(map, { { 27, "Bob" }, { 63, "Alice" }, { 84, "Romuald" } });
  thenMapContainsItems(other, { { 27, "Bob" }, { 63, "Alice" }, { 84, "Romuald" } });
}

BOOST_AUTO_TEST_CASE_TEMPLATE(GivenMapWithTwoItems_CheckPreIncrementationOnIterator,
                              K,  
                              TestedKeyTypes)
{
  Map<K> map = { { 1, "Bob" }, { 2, "Henry" } };
...
BOOST_AUTO_TEST_CASE_TEMPLATE(GivenMapWithTwoItems_CheckPostIncrementationOnIterator,
                              K,  
                              TestedKeyTypes)
{
  Map<K> map = { { 1, "Bob" }, { 2, "Henry" } };
